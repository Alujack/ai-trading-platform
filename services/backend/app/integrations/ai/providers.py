...
# The default sits at the backend root; AI_PROVIDER_OVERRIDES_PATH relocates it
# (a mounted volume in Docker, a tmp dir in tests).
_OVERRIDES_PATH = Path(
    os.environ.get(
        "AI_PROVIDER_OVERRIDES_PATH",
        str(Path(__file__).resolve().parents[3] / ".provider_overrides.json"),
    )
)
...
def _save_overrides() -> None:
    try:
        ...
        os.chmod(_OVERRIDES_PATH, 0o600)
