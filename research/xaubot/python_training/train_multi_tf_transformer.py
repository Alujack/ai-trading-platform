...
BATCH_SIZE = 1024        # Large batch for GPU efficiency (try 2048 if VRAM allows)
...
def train_epoch(model, loader, criterion, optimizer, device, scaler):
    ...
    for x_batch, y_batch in loader:
        ...
        # torch.cuda.amp.autocast in older APIs does not take device_type
...
def main():
    ...
    # Optimized DataLoaders for Lightning.ai GPU (8 workers, pinned memory, prefetch)
    NUM_WORKERS_TRAIN = 0 if sys.platform == 'win32' else 8
    NUM_WORKERS_VAL = 0 if sys.platform == 'win32' else 4
